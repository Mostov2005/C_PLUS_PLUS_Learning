...
//     cout << "\nСреднее арифметическое элементов ниже побочной диагонали: " << avg << "\n";
//
//     // 5. Память автоматически освободится умными указателями
// }
//
// // 1. Создание таблицы с умными указателями
// template<typename T>
// shared_ptr<shared_ptr<T>[]> Task2_4::createTable(int rows, int cols) {
//     auto table = make_shared<shared_ptr<T>[]>(rows);
//     for (int i = 0; i < rows; i++)
//         table[i] = make_shared<T[]>(cols);
//     return table;
// }
//
// // 2а. Заполнение нулями
// template<typename T>
// void Task2_4::fillZero(shared_ptr<shared_ptr<T>[]> table, int rows, int cols) {
//     for (int i = 0; i < rows; i++)
//         for (int j = 0; j < cols; j++)
//             table[i][j] = 0;
// }
...
// // 2б. Заполнение случайными числами
// template<typename T>
// void Task2_4::fillRandom(shared_ptr<shared_ptr<T>[]> table, int rows, int cols, T minVal, T maxVal) {
//     for (int i = 0; i < rows; i++)
//         for (int j = 0; j < cols; j++)
//             table[i][j] = minVal + rand() % (maxVal - minVal + 1);
// }
...
// // 2в. Заполнение с клавиатуры
// template<typename T>
// void Task2_4::fillKeyboard(shared_ptr<shared_ptr<T>[]> table, int rows, int cols) {
//     for (int i = 0; i < rows; i++)
//         for (int j = 0; j < cols; j++) {
//             cout << "table[" << i << "][" << j << "] = ";
//             cin >> table[i][j];
...
// // 2г. Заполнение из файла
// template<typename T>
// void Task2_4::fillFromFile(shared_ptr<shared_ptr<T>[]> table, int rows, int cols, const string &filename) {
//     ifstream fin(filename);
//     if (!fin) {
//         cout << "Файл " << filename << " не найден!\n";
//         return;
...
// // 3а. Вывод в консоль
// template<typename T>
// void Task2_4::printTable(shared_ptr<shared_ptr<T>[]> table, int rows, int cols) {
//     for (int i = 0; i < rows; i++) {
//         for (int j = 0; j < cols; j++)
//             cout << setw(5) << table[i][j];
//         cout << "\n";
...
// // 3б. Вывод в файл
// template<typename T>
// void Task2_4::printTableToFile(shared_ptr<shared_ptr<T>[]> table, int rows, int cols, const string &filename) {
//     ofstream fout(filename);
//     for (int i = 0; i < rows; i++) {
//         for (int j = 0; j < cols; j++)
//             fout << table[i][j] << " ";
...
// template<typename T>
// double Task2_4::averageBelowSecondaryDiagonal(shared_ptr<shared_ptr<T>[]> table, int rows, int cols) {
//     double sum = 0;
//     int count = 0;
//     for (int i = 0; i < rows; i++)
//         for (int j = 0; j < cols; j++)
...
// 5. Функция freeTable больше не нужна - память освобождается автоматически

//
// Основные изменения:
//
// T** → shared_ptr<shared_ptr<T>[]> - умные указатели вместо "голых"
//
// new → make_shared - безопасное создание
//
// Убрана freeTable - память освобождается автоматически
//
// Все функции теперь принимают умные указатели
//
// Преимущества:
//
// Нет утечек памяти
